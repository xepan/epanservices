...
class Controller_SideBarStatusFilter extends \AbstractController{
	...
	function init(){
		...
		if($this->add_all){
			$this->app->side_menu->addItem(['All','icon'=>'fa fa-cogs','badge'=>[$total,'swatch'=>' label label-primary label-circle pull-right']],$this->api->url(),['status','condition'])->setAttr(['title'=>'All']);
		}

		foreach ($this->owner->status as $s) {
			$this->app->side_menu->addItem([$s,'icon'=>'fa fa-cogs','badge'=>[$counts_redefined[$s],'swatch'=>' label label-primary label-circle pull-right']],$this->api->url(null,['status'=>$s]),['status'])->setAttr(['title'=>$s]);
		}
